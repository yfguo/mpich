...
def main():
    ...
    # generate MPII_CSEL_ enums for MPIR_ algorithms
    # print(json.dumps(G.algos))
    ...
    G.out.append(f"""
        void MPIR_Csel_process_env(void *csel_)
        {{
            csel_s * csel = (csel_s *) csel_;
            csel_node_s * tree_root = csel->u.root.tree;
    """)
    ...
    for item in enum_str:
        enums.append(f"MPII_CSEL_CONTAINER_TYPE__ALGORITHM__{item}")
    ...
    with open("src/mpi/coll/include/csel_container_gen.h", "w") as f:
        for e in enums:
            print(f"{e},", file=f)

    with open("src/mpi/coll/src/csel_constants_gen.h", "w") as f:
        for s in enum_str:
            print(f'"{s}",', file=f)
        # dump_coll(a, "persistent")
    # print(G.out)
    dump_c_file("src/mpi/coll/src/csel_env.c", G.out)
    subprocess.run(["./maint/code-cleanup.bash", "src/mpi/coll/src/csel_env.c"])
...
def dump_coll(name, blocking_type):
    if blocking_type == "blocking":
        gen_alg_names(name, blocking_type)
        dump_mpir_impl_blocking(name)
        # gen_mpir_cvars_blocking(name)
...
def gen_alg_names(name, blocking_type):
    """ Generate names for MPIR collective algorithms """
        ...
        for algo in G.algos[func_name + "-" + commkind]:
            if "allcomm" in algo:
                if commkind == "intra":
                    ...
                    continue
            enum_str.append(f"MPIR_{Name}_{commkind}_{algo["name"]}")
    if blocking_type == "blocking":
        enum_str.append(f"MPIR_{Name}_allcomm_nb")
...
def dump_mpir_impl_blocking(name):
    ...
    def dump_algo(algo, commkind):
        ...
        G.out.append(f"""
            csel_node_s * alg_env = csel_node_create(CSEL_NODE_TYPE__CONTAINER);
            CSEL_CONTAINER(alg_env)->id = MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_{Name}_{commkind}_{algo_name};
        """)

        if "extra_params" in algo:
            params = algo["extra_params"].split(", ")
            cvars = algo["cvar_params"].split(", ")
            for idx, p in enumerate(params):
                if cvars[idx] == "-":
                    continue
                G.out.append(f"""
                CSEL_CONTAINER(alg_env)->u.{name}.{commkind}_{algo_name}.{p} = MPIR_CVAR_{name.upper()}_{cvars[idx]};
                """)
    ...
    def dump_cases(commkind):
        ...
        for algo in G.algos[func_name + '-' + commkind]:
            if algo['name'] != "auto" and algo['name'] != "nb":
                G.out.append(f"""
                case {CVAR_PREFIX}_{algo["name"]}: {{
                    csel_node_s * algo_node = csel_tree_find(tree_root, is_{name});
                    algo_node = csel_tree_find(algo_node, is_{commkind}_comm);
                """)

                dump_algo(algo, commkind)

                # generate nodes to handle restrictions
                if 'restrictions' in algo:
                    restrictions = algo["restrictions"].replace('-', '_').split(", ")
                    idx = 0;
                    for r in restrictions:
                        G.out.append(f"""
                            csel_node_s * cond_{idx} = csel_node_create__{r}();

                            csel_node_s * alg_fallback_{idx} = csel_node_create(CSEL_NODE_TYPE__CONTAINER);
                            CSEL_CONTAINER(alg_fallback_{idx})->id = MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_{Name}_allcomm_nb;

                            cond_{idx}->failure = alg_fallback_{idx};
                        """)
                        if idx > 0:
                            G.out.append(f"""
                                cond_{idx - 1}->success = cond_{idx};
                            """)
                        idx = idx + 1

                    G.out.append(f"""
                        cond_{idx - 1}->success = alg_env;
                        csel_node_s * new_succ = cond_0;
                    """)

                else:
                    G.out.append(f"""
                        csel_node_s * new_succ = alg_env;
                    """)

                G.out.append(f"""
                    csel_node_s * old = csel_node_swap_success(tree_root, new_succ);
                    csel_tree_free(&old);
                    break;
                }}
                """)
        ...
        G.out.append(f"""
        case {CVAR_PREFIX}_nb: {{
            csel_node_s * algo_node = csel_tree_find(tree_root, is_{name});
            csel_node_s * algo_env = csel_node_create(CSEL_NODE_TYPE__CONTAINER);
            CSEL_CONTAINER(algo_env)->id = MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_{Name}_allcomm_nb;
            csel_node_s * old = csel_node_swap_success(algo_node, algo_env);
            csel_tree_free(&old);
            break;
        }}
        case {CVAR_PREFIX}_auto:
            break;
        default:
            MPIR_Assert(0);
        """)
    ...
    G.out.append(f"""
        switch (MPIR_CVAR_{NAME}_INTRA_ALGORITHM) {{
    """)
    dump_cases("intra")
    G.out.append(f"""
        }}
    ...
    if re.match(r'(scan|exscan|neighbor_)', name):
        ...
        G.out.append(f"""
            switch (MPIR_CVAR_{NAME}_INTER_ALGORITHM) {{
        """)
        dump_cases("inter")
        G.out.append(f"""
            }}
...
def gen_mpir_cvars_blocking(name):
    """ MPIR_Xxx_impl - """
    blocking_type = "blocking"
    func = G.FUNCS["mpi_" + name]
    params, args = get_params_and_args(func)
    ...
    def dump_cases(commkind):
        nonlocal need_fallback
        CVAR_PREFIX = f"MPIR_CVAR_{NAME}_{commkind.upper()}_ALGORITHM"
        G.out.append(f"""
              switch ({CVAR_PREFIX}) {{
        """)
        for algo in G.algos[func_name + '-' + commkind]:
            if algo['name'] != "auto" and algo['name'] != "nb":
                print(f"case {CVAR_PREFIX}_{algo['name']}: {{")
                if 'restrictions' in algo:
                    dump_fallback(algo)
                    need_fallback = True
                dump_algo(algo, commkind)
