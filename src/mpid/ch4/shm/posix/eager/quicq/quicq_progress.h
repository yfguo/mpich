...
MPL_STATIC_INLINE_PREFIX int MPIDI_POSIX_eager_progress(int vci, int *made_progress)
{
    ...
    *made_progress = 0;
    ...
    for (int vci_src = 0; vci_src < max_vcis; vci_src++) {
        ...
        for (int src_local_rank = 0; src_local_rank < MPIDI_POSIX_global.num_local;
             src_local_rank++) {
            if (src_local_rank == MPIDI_POSIX_global.my_local_rank) {
                continue;
            }
            terminal = &transport->recv_terminals[src_local_rank];
            if (terminal->last_seq == terminal->last_seq) {
                uint32_t new_seq = MPL_atomic_relaxed_load_uint32(&terminal->cntr->seq.a);
        ...
        for (int dst_local_rank = 0; dst_local_rank < MPIDI_POSIX_global.num_local;
             dst_local_rank++) {
            if (dst_local_rank == MPIDI_POSIX_global.my_local_rank) {
                continue;
            }
            ...
            if (terminal->last_ack < terminal->last_seq) {
                ...
                uint32_t new_ack = MPL_atomic_relaxed_load_uint32(&terminal->cntr->ack.a);
