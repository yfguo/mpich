...
static int init_transport(int vci_src, int vci_dst)
{
    ...
    transport->num_queues = MPIDI_POSIX_global.num_local - 1;
    int total_num_queues = MPIDI_POSIX_global.num_local * transport->num_queues;
    ...
    transport->send_terminals = (MPIDI_POSIX_eager_quicq_terminal_t *)
        MPL_malloc(MPIDI_POSIX_global.num_local * sizeof(MPIDI_POSIX_eager_quicq_terminal_t),
                   MPL_MEM_SHM);
    ...
    for (int remote_rank = 0; remote_rank < MPIDI_POSIX_global.num_local; remote_rank++) {
        int q_idx = 0;
