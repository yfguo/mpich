                       ...
                       MPI_Aint offset, int src_vci, int dst_vci, MPI_Aint * bytes_sent)
{
    ...
    if (unlikely(terminal->last_seq - terminal->last_ack >= transport->num_cells_per_queue)) {
        /* try ack completed messages to free the cells */
        ...
        if (new_ack != terminal->last_ack) {
            for (int i = terminal->last_ack; i < new_ack; i++) {
                cell_idx = MPIDI_POSIX_EAGER_QUICQ_CNTR_TO_IDX(i);
    ...
    cell_idx = MPIDI_POSIX_EAGER_QUICQ_CNTR_TO_IDX(terminal->last_seq);
    cell = terminal->cell_base + cell_idx * transport->cell_alloc_size;
