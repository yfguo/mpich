...
int MPIDI_POSIX_iqueue_init(int rank, int size)
{
    ...
    transport->total_num_cells = transport->num_cells * MPIDI_POSIX_global.num_local;

    mpi_errno = MPIDU_genq_shared_cell_pool_create(transport->size_of_cell, transport->num_cells,
                                                   transport->total_num_cells,
                                                   &transport->cell_pool);
  ...
  fn_fail:
    ...
    MPIDU_genq_shared_cell_pool_destroy(transport->cell_pool);
    goto fn_exit;
}
...
int MPIDI_POSIX_iqueue_finalize()
{
    ...
    mpi_errno = MPIDU_genq_shared_cell_pool_destroy(transport->cell_pool);
