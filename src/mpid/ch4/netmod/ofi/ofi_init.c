...
int MPIDI_OFI_mpi_init_hook(int rank, int size, int appnum, int *tag_bits, MPIR_Comm * init_comm)
{
    ...
    if (MPIDI_OFI_ENABLE_AM) {
        ...
        MPIDIG_am_reg_cb(MPIDI_OFI_AM_RDMA_READ_REJECT,
                         NULL, &MPIDI_OFI_am_rdma_read_reject_handler);
        MPIDIG_am_reg_cb(MPIDI_OFI_AM_RESEND, NULL, &MPIDI_OFI_am_resend_handler);
    }
