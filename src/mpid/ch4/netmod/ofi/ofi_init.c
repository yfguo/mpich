...
/* If inlined, ofi_impl.h shouldn't be included otherwise this will include
 * the entire inlined functions except for some type definitions, which breaks
 * the build. However, this is needed for non-inline builds to find some
...
int MPIDI_OFI_mpi_init_hook(int rank,
                            int size,
                            int appnum,
                            int *tag_ub,
