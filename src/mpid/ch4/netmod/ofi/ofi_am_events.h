                                                       ...
                                                       void *am_hdr, void *p_data)
{
    ...
    if (is_done) {
        ...
        /* only cache the request if we do expect more data. This is a corner case when
         * data+send_hdr is greater than eager limit but data+send_data_hdr is smaller than eager
         * limit which results in single segment of the pipeline. */
        if (!cache_rreq) {
