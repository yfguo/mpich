...
static int issue_deferred_am_isend_pipeline(MPIDI_OFI_deferred_am_isend_req_t * dreq)
{
    ...
    if (MPIDIG_REQUEST(dreq->sreq, req->lreq).offset == 0) {
        ...
        MPIDIG_REQUEST(child_sreq, req->lreq).context_id =
            MPIDIG_REQUEST(dreq->sreq, req->lreq).context_id;
    ...
    if (need_packing) {
        mpi_errno = MPIR_Typerep_pack(dreq->buf, dreq->count, dreq->datatype,
                                      MPIDIG_REQUEST(dreq->sreq, req->lreq).offset, send_buf,
