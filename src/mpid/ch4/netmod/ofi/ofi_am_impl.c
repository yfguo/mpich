...
int issue_deferred_am_isend_eager(MPIDI_OFI_deferred_am_isend_req_t * dreq)
{
    ...
    send_size = MPL_MIN(MPIDIG_REQUEST(dreq->sreq, send_ext->data_sz_left), send_size);
    MPIDI_OFI_am_create_send_request(dreq->sreq);
    ...
    if (need_packing) {
        ...
        mpi_errno = MPIR_Typerep_pack(dreq->buf, dreq->count, dreq->datatype,
                                      MPIDIG_REQUEST(dreq->sreq, send_ext->offset), send_buf,
                                      dreq->data_sz, &packed_size);
        MPIR_ERR_CHECK(mpi_errno);
        send_size = packed_size;

        MPIDI_OFI_AMREQUEST(dreq->sreq, send_req)->req_hdr.pack_buffer = send_buf;
    } else {
        send_buf = (char *) send_buf + MPIDIG_REQUEST(dreq->sreq, send_ext->offset);
        MPIDI_OFI_AMREQUEST(dreq->sreq, send_req)->req_hdr.pack_buffer = NULL;
    }
    ...
    MPIDIG_REQUEST(dreq->sreq, send_ext->data_sz_left) -= send_size;
    MPIDIG_REQUEST(dreq->sreq, send_ext->offset) += send_size;
...
static int issue_deferred_am_isend_pipeline(MPIDI_OFI_deferred_am_isend_req_t * dreq)
{
    ...
    if (MPIDIG_REQUEST(dreq->sreq, send_ext->plreq).seg_next > 1) {
    ...
    if (need_packing) {
        ...
        send_buf = (char *) (dreq->buf) + dt_true_lb + MPIDIG_REQUEST(dreq->sreq, send_ext->offset);
        MPIDI_OFI_AMREQUEST(dreq->sreq, send_req)->req_hdr.pack_buffer = NULL;
    }

    MPL_DBG_MSG_FMT(MPIDI_CH4_DBG_GENERAL, VERBOSE,
                    (MPL_DBG_FDEST,
                     "issue deferred pipeline seg for parent=0x%x, cc=%d, seg_next=%d",
                     dreq->sreq->handle, *(dreq->sreq->cc_ptr),
                     MPIDIG_REQUEST(dreq->sreq, send_ext->plreq).seg_next));
    ...
    MPIDIG_REQUEST(dreq->sreq, send_ext->data_sz_left) -= send_size;
    MPIDIG_REQUEST(dreq->sreq, send_ext->offset) += send_size;
    MPIDIG_REQUEST(dreq->sreq, send_ext->plreq).seg_next += 1;

    if (MPIDIG_REQUEST(dreq->sreq, send_ext->data_sz_left) == 0) {
...
int issue_deferred_am_isend_rdma_read(MPIDI_OFI_deferred_am_isend_req_t * dreq)
{
    ...
    MPIDIG_REQUEST(dreq->sreq, send_ext->data_sz_left) -= data_sz;
    MPIDIG_REQUEST(dreq->sreq, send_ext->offset) += data_sz;
