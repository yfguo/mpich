    ...
    if (!strncmp(resource, "pci:", strlen("pci:"))) {
        ...
        *resource_list_length = 1;
        ...
        while ((io_device = hwloc_get_next_osdev(topology, io_device))) {
            ...
            if (io_device->attr->osdev.type != HWLOC_OBJ_OSDEV_COPROC || (strlen(io_device->name)) == 3 || strncmp(io_device->name, "ze", strlen("ze")))
                continue;
        ...
        *resource_list_length = count;
        ...
        while ((io_device = hwloc_get_next_osdev(topology, io_device))) {
            ...
            if (io_device->attr->osdev.type != HWLOC_OBJ_OSDEV_COPROC || (strlen(io_device->name)) != 3)
                continue;
