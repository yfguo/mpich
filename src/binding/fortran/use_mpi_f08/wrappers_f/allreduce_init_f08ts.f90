...
subroutine MPIX_Allreduce_init_f08ts(sendbuf, recvbuf, count, datatype, op, comm, info, request, ierror)
    ...
    use :: mpi_c_interface, only : c_Datatype, c_Op, c_Comm, c_Info, c_Request
    ...
    if (c_int == kind(0)) then
        ierror_c = MPIR_Allreduce_init_cdesc(sendbuf, recvbuf, count, datatype%MPI_VAL, op%MPI_VAL, comm%MPI_VAL, info%MPI_VAL, request%MPI_VAL)
    else
        count_c = count
        ...
        comm_c = comm%MPI_VAL
        info_c = info%MPI_VAL
        ierror_c = MPIR_Allreduce_init_cdesc(sendbuf, recvbuf, count_c, datatype_c, op_c, comm_c, info_c, request_c)
