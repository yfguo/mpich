...
subroutine MPIX_Allgather_init_f08ts(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, &
    comm, ierror)
    ...
    if (c_int == kind(0)) then
        ierror_c = MPIR_Allgather_init_cdesc(sendbuf, sendcount, sendtype%MPI_VAL, recvbuf, recvcount, recvtype%MPI_VAL, &
            comm%MPI_VAL)
    else
        ...
        comm_c = comm%MPI_VAL
        ierror_c = MPIR_Allgather_init_cdesc(sendbuf, sendcount_c, sendtype_c, recvbuf, recvcount_c, recvtype_c, &
            comm_c)
    end if

    if (present(ierror)) ierror = ierror_c

end subroutine MPIX_Allgather_init_f08ts
