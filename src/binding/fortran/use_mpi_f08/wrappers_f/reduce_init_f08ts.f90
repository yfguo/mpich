...
subroutine MPIX_Reduce_init_f08ts(sendbuf, recvbuf, count, datatype, op, root, comm, ierror)
    ...
    if (c_int == kind(0)) then
        ierror_c = MPIR_Reduce_init_cdesc(sendbuf, recvbuf, count, datatype%MPI_VAL, op%MPI_VAL, root, comm%MPI_VAL)
    else
        count_c = count
        ...
        root_c = root
        comm_c = comm%MPI_VAL
        ierror_c = MPIR_Reduce_init_cdesc(sendbuf, recvbuf, count_c, datatype_c, op_c, root_c, comm_c)
    end if
