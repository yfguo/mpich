...
static int MPIR_Allgather_sched_intra_auto(const void *sendbuf, int sendcount,
                                           MPI_Datatype sendtype, void *recvbuf, int recvcount,
                                           MPI_Datatype recvtype, MPIR_Comm * comm_ptr,
                                           MPIR_TSP_sched_t * sched)
{
    int mpi_errno = MPI_SUCCESS;
    mpi_errno = MPIR_TSP_Iallgather_sched_intra_recexch(sendbuf, sendcount, sendtype, recvbuf,
                                                        recvcount, recvtype, comm_ptr, 1,
                                                        MPIR_CVAR_IALLGATHER_RECEXCH_KVAL, sched);
    return mpi_errno;
}
                        ...
                        MPIR_Info * info_ptr, MPIR_Request ** request, MPIR_Errflag_t * errflag)
{
    ...
    switch (MPIR_CVAR_IALLGATHER_INTRA_ALGORITHM) {
        ...
        default:
            mpi_errno =
                MPIR_Allgather_sched_intra_auto(sendbuf, sendcount, sendtype, recvbuf, recvcount,
...
/*@
 * MPIX_Allgather_init - Creates a nonblocking, persistent collective communication request for the Allgather operation.
 *
 * Input Parameters:
...
#ifdef HAVE_ERROR_CHECKING
    {
        mpi_errno =
            MPIR_Err_create_code(mpi_errno, MPIR_ERR_RECOVERABLE, __func__, __LINE__, MPI_ERR_OTHER,
                                 "**mpix_allgather_init",
                                 "**mpix_allgather_init %p %d %D %p %d %D %C %I %p", sendbuf,
