...
int MPIR_Csel_process_env(void *csel_)
{
    csel_s *csel = (csel_s *) csel_;
    printf("UPDATE TREE for ENV\n");
    // csel_node_s * node_allred = csel_tree_find(csel->u.root.tree, is_allreduce);
    // MPIR_Assert(node_allred);
    // csel_node_s * force_recexch = csel_tree_create_node(CSEL_NODE_TYPE__CONTAINER);
    // ((MPII_Csel_container_s *) force_recexch->u.cnt.container)->id = MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_Allreduce_intra_recexch;
    // csel_node_s * old = csel_tree_swap_success(node_allred, force_recexch);
    // Csel_print_tree(csel->u.root.tree);
    // Csel_print_rules(csel->u.root.tree);
    // csel_tree_free_tree(old);
    ...
    switch (MPIR_CVAR_BARRIER_INTRA_ALGORITHM) {
        case MPIR_CVAR_BARRIER_INTRA_ALGORITHM_k_dissemination:{
                csel_node_s *node = csel_tree_find(csel->u.root.tree, is_barrier);
                node = csel_tree_find(node, is_intra_comm);

                csel_node_s *alg = csel_tree_create_node(CSEL_NODE_TYPE__CONTAINER);
                CSEL_CONTAINER(alg)->id =
                    MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_Barrier_intra_k_dissemination;
                CSEL_CONTAINER(alg)->u.barrier.intra_k_dissemination.k =
                    MPIR_CVAR_BARRIER_DISSEM_KVAL;
                csel_node_s *old = csel_tree_swap_success(node, alg);
        ...
        case MPIR_CVAR_BARRIER_INTRA_ALGORITHM_recexch:{
                ...
                CSEL_CONTAINER(alg)->u.barrier.intra_recexch.k = MPIR_CVAR_BARRIER_RECEXCH_KVAL;
                CSEL_CONTAINER(alg)->u.barrier.intra_recexch.single_phase_recv =
        ...
        case MPIR_CVAR_BARRIER_INTRA_ALGORITHM_smp:{
                ...
                csel_node_s *alg1 = csel_tree_create_node(CSEL_NODE_TYPE__CONTAINER);
                CSEL_CONTAINER(alg1)->id =
                    MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_Barrier_intra_smp;

                csel_node_s *alg2 = csel_tree_create_node(CSEL_NODE_TYPE__CONTAINER);
                CSEL_CONTAINER(alg2)->id =
                    MPII_CSEL_CONTAINER_TYPE__ALGORITHM__MPIR_Barrier_allcomm_nb;

                csel_node_s *is_parent_comm =
                    csel_tree_create_node(CSEL_NODE_TYPE__OPERATOR__COMM_HIERARCHY);
                is_parent_comm->u.comm_hierarchy.val = MPIR_CSEL_COMM_HIERARCHY__PARENT;
                is_parent_comm->success = alg1;
                is_parent_comm->failure = alg2;

                csel_node_s *old = csel_tree_swap_success(node, is_parent_comm);
                csel_tree_free_tree(old);
                break;
            }
