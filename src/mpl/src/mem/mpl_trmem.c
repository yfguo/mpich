...
#ifdef MPL_HAVE__MM512_STREAM_SI512
...
{
    /* Anything less than 512 bytes is not worth optimizing */
    if (n <= 512) {
        memcpy(dest, src, n);
        return;
    }
    ...
    /* Copy 512 bytes at a time by unrolling a series of 64-byte streaming copies. */
    while (n >= 512) {
    ...
    /* Once there are fewer than 512 bytes left to be copied, copy a chunk of 256, 128 and 64 bytes (if
     * applicable). */
    if (n >= 256) {
        __m512i zmm0 = _mm512_loadu_si512((__m512i const *) (s + (64 * 0)));
        __m512i zmm1 = _mm512_loadu_si512((__m512i const *) (s + (64 * 1)));
